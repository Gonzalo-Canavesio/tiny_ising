...
static void update_red_black_grid(int (*write_grid)[2 + L],
  ...
  for (size_t i = 1; i < 1 + L / 2; ++i) {
    for (size_t j = 1; j < 1 + L; j += 8) {
      ...
      // Neighbor north
      __m256i neigh_n = _mm256_loadu_si256((__m256i *)&read_grid[(i - 1)][j]);
      // Neighbor south
      __m256i neigh_s = _mm256_loadu_si256((__m256i *)&read_grid[(i + 1)][j]);
...
static int calculate_red_black_grid(const int (*red_grid)[2 + L],
  ...
  for (size_t i = 1; i < 1 + L / 2; ++i) {
    for (size_t j = 1; j < 1 + L; j += 8) {
      ...
      // Neighbor north
      __m256i neigh_n = _mm256_loadu_si256((__m256i *)&black_grid[(i - 1)][j]);
      // Neighbor south
      __m256i neigh_s = _mm256_loadu_si256((__m256i *)&black_grid[(i + 1)][j]);
      // Neighbor medium (to avoid moving the elements in the animation)
