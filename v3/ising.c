...
void update(const float temp, int_least8_t grid[L][L]) {
  ...
  for (int_fast16_t i = 0; i < L; ++i) {
    for (int_fast16_t j = 0; j < L; ++j) {
      int_least8_t spin_old = grid[i][j];
      int_least8_t spin_new = (-1) * spin_old;

      // computing h_before
      int_least8_t spin_neigh_n = grid[(i + L - 1) % L][j];
      int_least8_t spin_neigh_e = grid[i][(j + 1) % L];
      int_least8_t spin_neigh_w = grid[i][(j + L - 1) % L];
      int_least8_t spin_neigh_s = grid[(i + 1) % L][j];
      int_least8_t h_before =
          -(spin_old * spin_neigh_n) - (spin_old * spin_neigh_e) -
          (spin_old * spin_neigh_w) - (spin_old * spin_neigh_s);

      // h after taking new spin
      int_least8_t h_after =
          -(spin_new * spin_neigh_n) - (spin_new * spin_neigh_e) -
          (spin_new * spin_neigh_w) - (spin_new * spin_neigh_s);

      int_least8_t delta_E = h_after - h_before;
...
double calculate(int_least8_t grid[L][L], int *M_max) {
  ...
  for (int_fast16_t i = 0; i < L; ++i) {
    for (int_fast16_t j = 0; j < L; ++j) {
      ...
      int_least8_t spin_neigh_n = grid[(i + 1) % L][j];
      int_least8_t spin_neigh_e = grid[i][(j + 1) % L];
      int_least8_t spin_neigh_w = grid[i][(j + L - 1) % L];
      int_least8_t spin_neigh_s = grid[(i + L - 1) % L][j];
