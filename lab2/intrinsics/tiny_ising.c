...
static void cycle(int (*black_grid)[L], int (*red_grid)[L],
  ...
  for (float temp = min; modifier * temp <= modifier * max; temp += step) {
    ...
    float e = 0.0f, e2 = 0.0f, e4 = 0.0f, m = 0.0f, m2 = 0.0f, m4 = 0.0f;
    for (size_t j = 0; j < TMAX; ++j) {
      update(temp, black_grid, red_grid);
      if (j % calc_step == 0) {
        float energy = 0.0f, mag = 0.0f;
        int M_max = 0;
...
int main(void) {
  ...
  // clear the grid
  int (*black_grid)[L] = calloc(L / 2, sizeof(int[L]));
  int (*red_grid)[L] = calloc(L / 2, sizeof(int[L]));
  init(black_grid);
  init(red_grid);
